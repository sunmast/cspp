	...
	partial class CsWalker
	{
		private string ExprSyntax(ExpressionSyntax expr, out TypeInfo exprType, bool isLeftValue = false)
		{
			...
			if (expr is TypeSyntax)
            {
                ...
                return this.WrapTypeName(exprType);
			}
			...
			{
				return this.ExprInitializerSyntax(expr as InitializerExpressionSyntax, out exprType);
			}
		...
		private string ExprMemberAccessSyntax(MemberAccessExpressionSyntax memberAccessExpression, bool isLeftValue, out TypeInfo exprType)
        {
			...
			if (exprType.Type is IArrayTypeSymbol)
			{
		...
		private string ExprAssignmentSyntax(AssignmentExpressionSyntax assignmentExpression, out TypeInfo exprType)
		{
			...
			string right = this.ExprSyntax(assignmentExpression.Right, out dummyType, false);
		...
		private string ExprCastSyntax(CastExpressionSyntax castExpression, out TypeInfo exprType)
		{
			...
			return string.Format("({0}){1}", this.WrapTypeName(exprType), this.ExprSyntax(castExpression.Expression, out dummyType));
		}
		...
		private string ExprArrayCreationSyntax(ArrayCreationExpressionSyntax arrayCreationExpression, out TypeInfo exprType)
		{
			...
			return string.Format("new_<{0}>({1})",
				this.SyntaxArrayType(arrayCreationExpression.Type, initializer == null || initializer.Expressions == null ? -1 : initializer.Expressions.Count),
                this.ExprSyntax(initializer, out this.dummyType));
		}

		private string ExprInitializerSyntax(InitializerExpressionSyntax initializerExpression, out TypeInfo exprType)
		{
            // E.g. x, y, z
			...
			{
                list.Add(this.ExprSyntax(expr, out this.dummyType));
			}
        ...
        private string ExprElementAccessSyntax(ElementAccessExpressionSyntax elementAccessExpression, bool isLeftValue, out TypeInfo exprType)
		{
            // E.g. x[index]
            TypeInfo objType;
            string obj = this.ExprSyntax(elementAccessExpression.Expression, out objType);
            ...
            if (objType.Type.Kind == SymbolKind.ArrayType || Util.IsAttributeDefined(objType.Type.GetAttributes(), "ImportedAttribute"))
            {
                ...
                else
                {
                    ...
                    return string.Format("{0}.{1}<{2}>({3})", obj, Consts.IndexOf, this.WrapTypeName(exprType), args);
                }
            }
            else
            {
        ...
        private string ExprSizeOfSyntax(SizeOfExpressionSyntax sizeOfExpression, out TypeInfo exprType)
        {
            ...
            return string.Format("sizeof({0})", this.WrapTypeName(sizeOfExpression.Type));
        }
        ...
        private string ExprSimpleLambdaSyntax(SimpleLambdaExpressionSyntax simpleLambdaExpression)
        {
            ...
            string body = this.ExprSyntax(simpleLambdaExpression.Body as ExpressionSyntax, out this.dummyType);
        ...
        private string ExprConditionalSyntax(ConditionalExpressionSyntax conditionalExpression, out TypeInfo exprType)
        {
            ...
            return string.Format("{0} ? {1} : {2}",
                this.ExprSyntax(conditionalExpression.Condition, out this.dummyType),
